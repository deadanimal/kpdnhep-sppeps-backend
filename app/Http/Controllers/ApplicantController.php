...
class ApplicantController extends Controller
{
    ...
    public function saveApplicationD(Request $request){
        $user = auth('api')->user();

             //check if exists
            $old = DB::table('Permohonan')->where('status_aktif',0)->where('id_pengguna',$user['id'])->where('jenis_permohonan','p3')->first(); 
